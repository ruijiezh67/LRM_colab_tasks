...
from src.modeling.modeling_stage1 import LatentSFTStage1Encoder
...
class MultiprocessTransformerWrapper:
    ...
    def _gen_per_process(
        ...
        model = LatentSFTStage1Encoder(encoder_name_or_path = encoder_name_or_path,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--encoder_name_or_path', type=str, required=True,
                        help='Base directory of the stage-1 encoder run; the '
                             'actual model is loaded from '
                             '<encoder_name_or_path>/checkpoint-<check_point>/hf')
    ...
    args.encoder_name_or_path = os.path.join(args.encoder_name_or_path, f'checkpoint-{check_point}/hf')
