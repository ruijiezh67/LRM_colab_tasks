...
def build_latent_token_induction_mask(
    ...
    pad_token_id: int,
    dtype: torch.dtype | None = None,   # None -> bool mask; other -> float additive mask (-inf)
) -> torch.Tensor:
    """
...
def prepare_single_example(examples, model, encoder_name_or_path, compression_rate):
    ...
    elif 'qwen' in encoder_name_or_path.lower():
        messages = [
...
def left_pad_2d(sequences, fill_value, dtype=torch.long):
    """Left-pad a list of 1D lists to the same length, return tensor [B, max_len]."""
...
class MultiprocessTransformerWrapper:
    ...
    def _gen_per_process(
        ...
        model = LatentSFTStage1Union(encoder_name_or_path = encoder_name_or_path,
        ...
        with torch.no_grad():
            while True:
                ...
                # --- Prepare all examples in the batch ---
                prepared = []
                ...
                # --- Left-pad input_ids for generation (decoder side) ---
                all_input_ids = [p['input_ids'] for p in prepared]
                input_ids_tensor = left_pad_2d(all_input_ids, fill_value=pad_token_id).to(device)  # [bs, max_input_len]
                input_attn_mask = (input_ids_tensor != pad_token_id).long()  # [bs, max_input_len]
                # Also mark -100 positions as attended (they are latent placeholders, not padding)
                input_attn_mask[input_ids_tensor == -100] = 1
                
                # --- Right-pad cot_ids for encoder (standard right-padding) ---
                all_cot_ids = [p['cot_ids'] for p in prepared]
                cot_ids_tensor = right_pad_2d(all_cot_ids, fill_value=pad_token_id).to(device)  # [bs, max_cot_len]
                ...
                # --- Encoder: compress ---
                compress_embedding = model._compress(cot_ids_tensor, cot_attention_mask)  # [bs, max_cot_len, h]
                
                compress_mask = (cot_ids_tensor == model.compress_token_id)  # [bs, max_cot_len]
                decoder_mask = (input_ids_tensor == -100)                     # [bs, max_input_len]
                ...
                input_embeddings = model.decoder.model.model.embed_tokens(input_ids_for_embed)  # [bs, max_input_len, h]
                
                # --- Replace latent positions with compressed embeddings ---
                for b in range(bs):
                    compress_idx = compress_mask[b].nonzero(as_tuple=False).squeeze(-1)
                    decoder_idx = decoder_mask[b].nonzero(as_tuple=False).squeeze(-1)
                    assert compress_idx.shape[0] == decoder_idx.shape[0], \
                        f"Sample {b}: compress count {compress_idx.shape[0]} != decoder count {decoder_idx.shape[0]}"
    ...
    def _gen(
        ...
        # Enqueue batches of sorted data
        num_batches = 0
        ...
        # Collect results (keyed by sorted batch start index)
        id_gen_texts = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--check_point', type=str, required=True,
                        help='Union-tuning LoRA checkpoint step to evaluate, e.g. "1885"')
    ...
    parser.add_argument('--lora_path', type=str, required=True,
                        help='Base directory of the stage-1 union (LoRA) run; the '
                             'actual adapter is loaded from '
                             '<lora_path>/checkpoint-<check_point>/lora_adapter')
    ...
    args.lora_path = os.path.join(args.lora_path, f'checkpoint-{check_point}/lora_adapter')
    ...
    ## evaluate
    for i in range(len(data)):
        pred: str = extract_answer(pred_str=gen_texts[i])
        gt = data[i]["answer"]
        results.append(check_is_correct(pred, gt=gt))
    ...
    with open(os.path.join(args.save_path, f'eval_result_{args.dataset}.txt'), "a", encoding="utf-8") as f:
        f.write('=======================================================\n')
        f.write(f'Scores: {sum(results)/len(results)} check_point: {check_point} mex_new_tokens: {args.max_new_tokens}'+'\n')
