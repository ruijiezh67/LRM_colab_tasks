...
def main():
    ...
    # (读一次即可, 并显式 utf-8: 原来读三遍且无 encoding, 在非 UTF-8 默认编码的机器上会崩)
    with open(configs.val_path, encoding="utf-8") as _vf:
        _val_data = json.load(_vf)
    question_val = [d["question"] for d in _val_data]
    answers_val = [d["answer"].replace(",", "").strip() for d in _val_data]
    cot_val = ["\n".join(d["steps"]) for d in _val_data]
