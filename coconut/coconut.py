...
def _trim_cache(kv_cache, seq_end: int) -> DynamicCache:
    """Trim a DynamicCache (or legacy list-of-tuples) to ``[:, :, :seq_end, :]``."""
    trimmed = DynamicCache()
    for layer_idx, (k, v) in enumerate(kv_cache):
        trimmed.update(k[:, :, :seq_end, :], v[:, :, :seq_end, :], layer_idx)
    return trimmed
    ...
    def forward(self, input_ids, attention_mask, labels, position_ids, **kwargs):
        ...
        for pass_idx in range(max_n_latents):
            ...
            if kv_cache == None:
                ...
                past_key_values = _trim_cache(kv_cache, next_compute_range[0])
        ...
        outputs = self.base_causallm(
            ...
            past_key_values=(
                _trim_cache(kv_cache, next_compute_range[0])
                if kv_cache
                else None
            ),
            output_hidden_states=True,
