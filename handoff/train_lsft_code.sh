...
pip -q install "transformers==4.51.1" "deepspeed==0.17.0" "tokenizers<0.22" "peft==0.15.2" "datasets==3.6.0" pylatexenc word2number omegaconf accelerate huggingface_hub
...
python "generate_latent_soft_label_lora_batch.py" --encoder_model_path "$ENC" --decoder_model_path "$DEC" --lora_path "$UNI" --save_path "$LSFT/data/soft_code" --data_path "$LSFT/data/code-train.jsonl" --mp_size 1 --batch_size 16 --dtype bfloat16 --compression_rate 2 --topk_interpolation 10
python "merge_lora.py" --base_model_path "$DEC" --lora_path "${DEC_LORA:-$UNI}" --output_path "$LSFT/output/merged" --output_subdir decoder_hf --dtype bfloat16 --attn_implementation sdpa --device_map auto
MERGED="$LSFT/output/merged/decoder_hf"

echo "[7/8] stage2 (final base)"
python - "$LSFT" "$MERGED" <<'PY'
