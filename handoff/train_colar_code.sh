...
echo "[4/5] 数据 → coding_mix (qsa 格式)$([ "$NROW" -gt 0 ] && echo ", 取前 $NROW 验证")"
...
PY
  ...
  [ -z "$CKPT" ] && CKPT=$(ls -t "$WORK"/colar/logs/colar/qsa-coding_mix/*$TAG*/checkpoints/last.ckpt 2>/dev/null | head -1)
