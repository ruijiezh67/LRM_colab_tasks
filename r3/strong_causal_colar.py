...
HERE = Path(__file__).resolve().parent; KT = HERE.parent.parent.parent.parent
...
def main():
    ...
    ap.add_argument("--pool", default=str(KT / "common" / "question_sets" / "gsmllm_pool.json"))
    ...
    EMB_STD_RAW = os.environ.get("COLAR_EMB_STD", "0.018")   # 数值; 或 "auto"(R1 换底座, 从模型嵌入 std 估)
    ...
    # R1 换底座诊断: emb_std/sep 随底座变 -> 显式暴露 + 断言 sep 有效(否则潜链恒 MAX_LAT)
    emb_model_std = float(emb.weight.detach().float().std())
    EMB_STD = emb_model_std if str(EMB_STD_RAW).lower() == "auto" else float(EMB_STD_RAW)
    unk_id = getattr(tok, "unk_token_id", None)
    assert sep_id is not None and int(sep_id) >= 0, "'###' 未解析为 token id(检查底座 tokenizer)"
    if unk_id is not None and sep_id == unk_id:
        print("!! [warn] '###' -> unk_token: 底座分词器可能不把 '###' 当单一 token, 潜链停止判据会失效", flush=True)
    print(f"[strong_causal] base={os.path.basename(BASE)} H={H} sep={sep_id} "
          f"emb_std(used)={EMB_STD:.5f} emb_std(model)={emb_model_std:.5f} compress={COMPRESS}", flush=True)
